...
const Navbar = () => {
  ...
  return (
    <header
      className={
        "fixed inset-x-6 items-center top-4 transition-all duration-300  z-50" +
        (isScrolled ? " -translate-y-1" : " translate-y-0")
      }
    >
      <nav
        className={
          "p-6 py-2 rounded-xl flex justify-between items-center transition-all duration-300  box-border w-full" +
          (isScrolled
            ? " bg-surface-el/85 backdrop-blur-[28px] border border-border"
            : " bg-transparent")
        }
      >
        <h1 className="font-display text-xl tracking-tight text-foreground flex items-center">
          <a
            href="#hero"
            className="inline-block bg-cta rounded-xl p-2 mr-2 shadow-[0_0_40px_3px_var(--color-primary-glow)]/20"
          >
            <svg
        ...
        {isDesktop ? (
          <>
            ...
            <button className="btn-glow p-3.5 py-2 mt-2 text-sm font-semibold rounded-xl mb-1">
      ...
      >
        ...
        <button className="btn-glow w-full p-2.5 mt-2 text-sm font-semibold rounded-xl">
          <a
            href="#contact"
            onClick={() => {
              setIsOpen(false);
